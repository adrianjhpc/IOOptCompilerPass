...
module {
  ...
  // CHECK-LABEL: cir.func @test_read_step_3
  cir.func @test_read_step_3(%fd: !s32i, %buf: !cir.ptr<!s8i>) {
    ...
    // CHECK: io.batch_readv {{.*}}, {{.*}}, {{.*}}, %[[TRIP_COUNT_1]]
    cir.return
  }

  // --- TEST 2: Step of 7 with bound of 50 (50 / 7 = 7.14 -> 8 iterations) ---
  // CHECK-LABEL: cir.func @test_read_step_7
  cir.func @test_read_step_7(%fd: !s32i, %buf: !cir.ptr<!s8i>) {
    %0 = cir.alloca !s32i, !cir.ptr<!s32i>, ["i", init] {alignment = 4 : i64}
    %zero = cir.const #cir.int<0> : !s32i
    cir.store align(4) %zero, %0 : !s32i, !cir.ptr<!s32i>
    ...
    // CHECK: io.batch_readv {{.*}}, {{.*}}, {{.*}}, %[[TRIP_COUNT_2]]
    cir.return
  }

  // --- TEST 3: Multiplicative Loop (MUST NOT BE OPTIMIZED) ---
  // CHECK-LABEL: cir.func @test_read_step_mul
  cir.func @test_read_step_mul(%fd: !s32i, %buf: !cir.ptr<!s8i>) {
    %0 = cir.alloca !s32i, !cir.ptr<!s32i>, ["i", init] {alignment = 4 : i64}
