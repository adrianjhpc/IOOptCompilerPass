...
def teardown_db(bin_dir, db_dir):
    ...
    run_cmd(f"{bin_dir}/pg_ctl -D {db_dir} stop", check=False, capture=True)
    if os.path.exists(db_dir):
        shutil.rmtree(db_dir)
...
def main():
    ...
    for i in range(args.runs):
        ...
        setup_db(args.bin_dir, args.db_dir)
